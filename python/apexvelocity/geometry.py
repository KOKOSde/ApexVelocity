    ...
    def __init__(self, center_lat: float, center_lon: float):
        ...
        if _HAS_PYPROJ:
            ...
            self._to_latlon = Transformer.from_crs(
                f"EPSG:{self.utm_zone.epsg_code}", "EPSG:4326", always_xy=True
            )
        else:
            self._to_utm = None
...
def _catmull_rom_point(
    ...
    # Catmull-Rom formula
    def lerp(
        a: Tuple, b: Tuple, ta: float, tb: float, tc: float
    ) -> Tuple[float, float]:
        if abs(tb - ta) < 1e-10:
            return a
        ratio = (tc - ta) / (tb - ta)
...
def _interpolate_catmull_rom(
    x_arr: List[float], y_arr: List[float], target_spacing: float
    ...
    s_extended = (
        [s_arr[0] - (s_arr[1] - s_arr[0])]
        + s_arr
        + [s_arr[-1] + (s_arr[-1] - s_arr[-2])]
    )
    ...
    for s_target in s_out:
        ...
        p3 = (
            pts_extended[seg_idx + 3]
            if seg_idx + 3 < len(pts_extended)
            else pts_extended[-1]
        )
...
def _interpolate_linear(
    ...
    # Ensure endpoint
    if len(x_new) > 0 and (
        abs(x_new[-1] - x_arr[-1]) > 0.01 or abs(y_new[-1] - y_arr[-1]) > 0.01
    ):
...
def validate_path_alignment(
    raw_points: List[Dict], smoothed_points: List[Dict], max_offset_m: float = 3.0
) -> bool:
    """
...
def recompute_curvature(points: List[Dict], smooth_window: int = 3) -> List[Dict]:
    ...
    for i in range(n):
        if i == 0:
            ...
            if n > 2:
                k = _menger_curvature(
                    (x_arr[0], y_arr[0]), (x_arr[1], y_arr[1]), (x_arr[2], y_arr[2])
                )
            else:
                k = 0.0
        elif i == n - 1:
...
def get_path_stats(points: List[Dict]) -> Dict:
    ...
    spacings = []
    for i in range(1, n):
        d = points[i].get("distance_along_m", 0) - points[i - 1].get(
            "distance_along_m", 0
        )
        if d > 0:
            spacings.append(d)
