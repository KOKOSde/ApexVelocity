...
class ApexVelocityEnv:
    ...
    def __init__(
        ...
        self.observation_space = spaces.Box(
            low=np.array(
                [0, 0] + [-1.0] * lookahead_points + [0, 0, 0], dtype=np.float32
            ),
            high=np.array(
                [100, 100] + [1.0] * lookahead_points + [1, path_length, 1],
                dtype=np.float32,
            ),
            dtype=np.float32,
        )
    ...
    def _get_curvature_at(self, position: float) -> float:
        ...
        for i in range(len(self.path_positions) - 1):
            if self.path_positions[i] <= position < self.path_positions[i + 1]:
                ...
                return (1 - t) * self.path_curvatures[i] + t * self.path_curvatures[
                    i + 1
                ]
    ...
    def _get_observation(self) -> np.ndarray:
        ...
        for i in range(self.lookahead_points):
            dist = self.position + (i + 1) * (
                self.lookahead_distance / self.lookahead_points
            )
        ...
        obs = np.array(
            ...
            + lookahead
            + [self.mu, dist_remaining, energy_norm],
            dtype=np.float32,
        )
    ...
    def render(self):
        ...
        if self.render_mode == "human":
            print(
                f"Position: {self.position:.1f}m, Speed: {self.speed:.1f}m/s, "
