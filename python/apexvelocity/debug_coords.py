...
def print_path_debug(
    path_points: List[Dict], label: str = "PATH", n_samples: int = 5, file=sys.stdout
):
    """
    ...
    if has_lat_lon:
        ...
        print(
            f"    Latitude:  {min(lats):.6f} to {max(lats):.6f} (range: {max(lats)-min(lats):.6f}°)",
            file=file,
        )
        print(
            f"    Longitude: {min(lons):.6f} to {max(lons):.6f} (range: {max(lons)-min(lons):.6f}°)",
            file=file,
        )
    ...
    if "distance_along_m" in path_points[-1]:
        ...
        print(
            f"\n  Total distance: {total_dist:.1f} m ({total_dist/1000:.2f} km)",
            file=file,
        )
    ...
    print(
        f"  {'idx':<5} {'lat':<12} {'lon':<14} {'dist_m':<10} {'curv':<10} {'surface'}",
        file=file,
    )
    ...
    for i in range(min(n_samples, n)):
        ...
        print(
            f"  {i:<5} {lat:<12.6f} {lon:<14.6f} {dist:<10.1f} {curv:<10.6f} {surf}",
            file=file,
        )
    ...
    if n > n_samples:
        ...
        surf = p.get("surface_type", "?")
        print(
            f"  {n-1:<5} {lat:<12.6f} {lon:<14.6f} {dist:<10.1f} {curv:<10.6f} {surf}",
            file=file,
        )
    ...
    if n < 50:
        issues.append(
            f"  ⚠️  Low point count ({n}) - may cause jagged/chunky appearance"
        )
    ...
    if n > 1 and has_lat_lon:
        ...
        for i in range(1, n):
            dlat = (
                path_points[i].get("lat", 0) - path_points[i - 1].get("lat", 0)
            ) * 111320
            dlon = (
                (path_points[i].get("lon", 0) - path_points[i - 1].get("lon", 0))
                * 111320
                * math.cos(math.radians(avg_lat))
            )
        ...
        print(
            f"  Point spacing: min={min_spacing:.1f}m, max={max_spacing:.1f}m, avg={avg_spacing:.1f}m",
            file=file,
        )

        if max_spacing > 50:
            issues.append(
                f"  ⚠️  Large gaps between points (max {max_spacing:.0f}m) - interpolation may help"
            )
...
def print_viz_segment_debug(
    path_data: List[Dict], label: str = "VIZ", n_samples: int = 3, file=sys.stdout
):
    """
        ...
        if "path" in seg:
            ...
            if len(path_coords) >= 2:
                ...
                if 37 < lat1 < 38 and -123 < lon1 < -122:
                    print(
                        f"    ✓ Looks like San Francisco coordinates (correct order)",
                        file=file,
                    )
                # Nürburgring check
                elif 50 < lat1 < 51 and 6 < lon1 < 8:
                    print(
                        f"    ✓ Looks like Nürburgring coordinates (correct order)",
                        file=file,
                    )
                # Check for swapped coords
                elif 37 < lon1 < 38 and -123 < lat1 < -122:
                    print(
                        f"    ⚠️  SWAPPED! lat/lon appear to be in wrong order!",
                        file=file,
                    )
