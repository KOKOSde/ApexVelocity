...
class SegmentFeatures:
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'curve_radius_m': round(self.curve_radius_m, 1) if self.curve_radius_m < 1e6 else '∞',
            ...
            'speed_limit_margin_kmh': round(self.speed_limit_margin_kmh, 1) if self.speed_limit_margin_kmh else None,
            ...
            'time_to_next_brake_s': round(self.time_to_next_brake_s, 1) if self.time_to_next_brake_s else None,
        }
...
def compute_physics_limit_speed(curvature: float, mu: float,
...
def determine_recommended_action(
    ...
    if friction_usage > CRITICAL_FRICTION_USAGE:
        conf = 1.0 - (friction_usage - CRITICAL_FRICTION_USAGE) / (1.0 - CRITICAL_FRICTION_USAGE)
        return "BRAKE", min(1.0, 1.0 - conf * 0.3), f"Critical friction ({friction_usage:.0%})"
    ...
    if friction_usage > HIGH_FRICTION_USAGE:
        margin = (CRITICAL_FRICTION_USAGE - friction_usage) / (CRITICAL_FRICTION_USAGE - HIGH_FRICTION_USAGE)
        return "EASE_OFF", 0.7 + 0.2 * margin, f"High friction usage ({friction_usage:.0%})"
...
def analyze_profile(
    ...
    # Action tracking
    action_distances = {"BRAKE": 0, "EASE_OFF": 0, "COAST": 0, 
                        "HOLD": 0, "ACCELERATE": 0, "SMOOTH": 0}
    ...
    for i, pt in enumerate(path_points):
        ...
        # Segment length
        if i > 0:
            seg.segment_length_m = max(0.1, seg.distance_along_m - path_points[i-1].get('distance_along_m', 0))
        
        # Grade
        if i > 0 and seg.segment_length_m > 0.1:
            prev_elev = path_points[i-1].get('z_m', path_points[i-1].get('elevation_m', 0))
        ...
        if i < len(path_points) - 1:
            ...
            heading = _compute_heading(seg.lat, seg.lon, 
        ...
        seg.speed_limit_kmh = pt.get('speed_limit_kmh', pt.get('osm_speed_limit_kmh', pt.get('maxspeed', None)))
        seg.lane_count = pt.get('lane_count', pt.get('lanes', None))
        seg.is_intersection_segment = pt.get('is_intersection', False)
        
        # ----- Friction -----
        seg.friction_mu_effective = get_friction_coefficient(seg.surface_type, condition) * friction_multiplier
        ...
        try:
            global_cap = float(os.environ.get("APEXVELOCITY_MAX_DISPLAY_SPEED_KMH", "160"))
        except Exception:
            global_cap = 160.0
        ...
        if i > 0 and seg.segment_length_m > 0:
            prev_speed = path_points[i-1].get('speed_mps', path_points[i-1].get('v_profile', seg.speed_mps))
            ...
            if avg_speed > 0.1:
                ...
                if len(segments) > 0 and dt > 0:
                    prev_seg = segments[-1]
                    seg.longitudinal_jerk_mps3 = (seg.longitudinal_accel_mps2 - prev_seg.longitudinal_accel_mps2) / dt
                    seg.lateral_jerk_mps3 = (seg.lateral_accel_mps2 - prev_seg.lateral_accel_mps2) / dt
                    seg.jerk_magnitude_mps3 = math.sqrt(seg.longitudinal_jerk_mps3**2 + seg.lateral_jerk_mps3**2)
        
        seg.total_accel_g = math.sqrt(seg.lateral_g**2 + seg.longitudinal_g**2)
        
        # ----- Friction Usage -----
        max_accel = seg.friction_mu_effective * GRAVITY
        actual_accel = math.sqrt(seg.lateral_accel_mps2**2 + seg.longitudinal_accel_mps2**2)
        if max_accel > 0:
        ...
        if i > 0 and seg.segment_length_m > 0:
            ...
            if seg.longitudinal_accel_mps2 < -0.5 or seg.grade_percent < -2:
                ...
                if dv < 0:
                    kinetic_regen = 0.5 * vehicle_mass_kg * (path_points[i-1].get('speed_mps', seg.speed_mps)**2 - seg.speed_mps**2)
                    seg.regen_potential_j = kinetic_regen * 0.7  # 70% regen efficiency
            
            if seg.grade_percent < -1:
                grade_regen = vehicle_mass_kg * GRAVITY * abs(seg.grade_percent/100) * seg.segment_length_m * 0.7
        ...
        if seg.speed_ratio < 0.75 and seg.friction_usage < 0.4 and seg.grade_percent < 1:
            seg.coasting_opportunity = True
        
        # ----- Comfort -----
        lat_discomfort = max(0, seg.lateral_g - comfort_g_threshold) / comfort_g_threshold
        long_discomfort = max(0, abs(seg.longitudinal_g) - COMFORT_LONGITUDINAL_G_THRESHOLD) / COMFORT_LONGITUDINAL_G_THRESHOLD
        jerk_discomfort = max(0, seg.jerk_magnitude_mps3 - comfort_jerk_threshold) / comfort_jerk_threshold
        
        seg.comfort_score = min(1.0, (lat_discomfort + long_discomfort + jerk_discomfort) / 3)
        ...
        seg.target_speed_kmh = min(seg.physics_limit_speed_kmh * 0.85, 
                                    seg.speed_kmh * comfort_limit_factor)
        if seg.speed_limit_kmh:
            seg.target_speed_kmh = min(seg.target_speed_kmh, seg.speed_limit_kmh)
        
        # ----- Recommended Action -----
        seg.recommended_action, seg.action_confidence, seg.action_reason = determine_recommended_action(
            seg.speed_ratio, seg.friction_usage, seg.lateral_g,
            seg.longitudinal_accel_mps2, seg.grade_percent, 
            seg.comfort_score, seg.curvature_1pm
        )
    ...
    if speeds:
        ...
        analysis.speed_std_kmh = math.sqrt(sum((s - mean_speed)**2 for s in speeds) / len(speeds))
    ...
    if analysis.total_distance_km > 0:
        analysis.energy_kwh_per_km = analysis.total_energy_kwh / analysis.total_distance_km
    ...
    if analysis.total_distance_km > 0:
        analysis.comfort_violations_per_km = total_comfort_violations / analysis.total_distance_km
        analysis.pct_uncomfortable_distance = uncomfortable_distance / (total_distance if total_distance > 0 else 1)
    
    # Action distribution
    if total_distance > 0:
        analysis.brake_fraction = (action_distances["BRAKE"] + action_distances["EASE_OFF"]) / total_distance
        ...
        analysis.hold_fraction = (action_distances["HOLD"] + action_distances["SMOOTH"]) / total_distance
    ...
    if overspeed_deltas:
        analysis.avg_speed_over_limit_kmh = sum(overspeed_deltas) / len(overspeed_deltas)
    if total_distance > 0:
    ...
    curv_var = sum((c - analysis.avg_curvature)**2 for c in curvatures) / len(curvatures) if curvatures else 0
    curv_factor = min(1.0, curv_var * 10000)  # Scale variance
    grade_factor = min(1.0, analysis.max_grade_percent / 15)  # 15% = max difficulty
    int_factor = min(1.0, (analysis.intersection_count / max(1, analysis.total_distance_km)) / 10)  # 10/km = max
    comfort_factor = min(1.0, analysis.comfort_violations_per_km / 5)  # 5/km = max difficulty
    
    analysis.difficulty_score = (curv_factor + grade_factor + int_factor + comfort_factor) / 4
    
    # Safety margin: inverse of friction usage and lateral G
    analysis.safety_margin_score = max(0, 1.0 - (analysis.avg_friction_usage + analysis.p95_lateral_g) / 2)
