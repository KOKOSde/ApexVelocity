...
def create_hill_path(
    ...
    for i in range(flat_after):
        path_points.append({
            'lat': base_lat + (flat_before + uphill_length + flat_top + downhill_length + i) * 0.0001,
            'lon': base_lon,
            'z_m': elevation,
            'curvature': 0,
            'distance_along_m': distance,
...
class TestTurnPath:
    ...
    def test_lateral_g_in_turn(self):
        ...
        turn_max_g = max(s.lateral_g for s in analysis.segments[approach:approach+turn_points])
        
        assert turn_max_g > approach_max_g * 5, \
    ...
    def test_comfort_score_higher_in_turn(self):
        ...
        approach_comfort = sum(s.comfort_score for s in analysis.segments[:approach]) / approach
        turn_comfort = sum(s.comfort_score for s in analysis.segments[approach:approach+turn_points]) / turn_points
        
        assert turn_comfort > approach_comfort, \
    ...
    def test_friction_usage_higher_in_turn(self):
        ...
        turn_friction = max(s.friction_usage for s in analysis.segments[approach:approach+turn_points])
        approach_friction = max(s.friction_usage for s in analysis.segments[:approach])
        
        assert turn_friction > approach_friction * 2, \
    ...
    def test_brake_recommended_in_tight_turn(self):
        ...
        turn_segments = analysis.segments[approach:approach+turn_points]
        brake_or_ease = sum(1 for s in turn_segments 
...
class TestHillPath:
    ...
    def test_grade_positive_uphill(self):
        ...
        uphill_grades = [s.grade_percent for s in analysis.segments[uphill_start+1:uphill_end]]
        avg_uphill_grade = sum(uphill_grades) / len(uphill_grades) if uphill_grades else 0
        
        assert avg_uphill_grade > 5.0, f"Expected positive grade uphill, got {avg_uphill_grade}%"
    ...
    def test_grade_negative_downhill(self):
        ...
        downhill_grades = [s.grade_percent for s in analysis.segments[downhill_start+1:downhill_end]]
        avg_downhill_grade = sum(downhill_grades) / len(downhill_grades) if downhill_grades else 0
        
        assert avg_downhill_grade < -5.0, f"Expected negative grade downhill, got {avg_downhill_grade}%"
    ...
    def test_elevation_gain_tracked(self):
        ...
        assert abs(analysis.total_elevation_gain_m - expected_gain) < 2.0, \
