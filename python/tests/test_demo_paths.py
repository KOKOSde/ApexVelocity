...
class TestLombardStreet:
    ...
    def test_path_length_in_range(self):
        """Lombard Street crooked section should be roughly 130-250m."""
        ...
        assert (
            130 <= total_dist <= 250
        ), f"Path length {total_dist:.0f}m should be 130-250m"

    def test_path_has_enough_points(self):
        """Smoothed path should have at least 40 points."""
        from examples.demo_san_francisco import create_lombard_path

        path = create_lombard_path()
        n_points = len(path)

        assert n_points >= 40, f"Path has {n_points} points, expected at least 40"
    ...
    def test_coordinates_in_san_francisco(self):
        ...
        assert (
            37.801 <= min(lats) <= max(lats) <= 37.803
        ), f"Latitudes {min(lats):.4f} to {max(lats):.4f} should be ~37.802"
        assert (
            -122.421 <= min(lons) <= max(lons) <= -122.418
...
class TestNurburgring:
    ...
    def test_path_length_in_range(self):
        ...
        assert 17 <= total_km <= 25, f"Track length {total_km:.1f}km should be 17-25km"
    ...
    def test_path_has_enough_points(self):
        ...
        assert n_points >= 500, f"Track has {n_points} points, expected at least 500"
    ...
    def test_coordinates_in_germany(self):
        ...
        assert (
            50.31 <= min(lats) <= max(lats) <= 50.36
        ), f"Latitudes {min(lats):.4f} to {max(lats):.4f} should be ~50.33"
        assert (
            6.90 <= min(lons) <= max(lons) <= 7.05
    ...
    def test_track_forms_loop(self):
        ...
        dlon = (
            (end["lon"] - start["lon"]) * 111320 * math.cos(math.radians(start["lat"]))
        )
        gap = math.sqrt(dlat ** 2 + dlon ** 2)

        assert gap < 100, f"Track gap {gap:.0f}m should be < 100m for a closed loop"
...
class TestSmoothingEffect:
    ...
    def test_sf_smooth_vs_raw(self):
        ...
        from examples.demo_san_francisco import (
            create_raw_lombard_path,
            create_lombard_path,
        )
    ...
    def test_nurburgring_smooth_vs_raw(self):
        ...
        from examples.benchmark_nurburgring import (
            create_raw_track_path,
            create_track_path,
        )
