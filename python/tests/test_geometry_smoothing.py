...
class TestStraightLine:
    ...
    def test_straight_line_curvature_is_zero(self):
        ...
        assert (
            max_curv < 0.001
        ), f"Max curvature {max_curv} should be < 0.001 for straight line"
...
class TestCircularArc:
    ...
    def test_quarter_circle_curvature(self):
        ...
        n = len(smooth_path)
        middle_curvatures = [
            smooth_path[i]["curvature"] for i in range(n // 4, 3 * n // 4)
        ]
        ...
        assert (
            diff_pct < 30
        ), f"Avg curvature {avg_curv:.4f} vs expected {expected_curv:.4f} ({diff_pct:.0f}% diff)"
    ...
    def test_tight_turn_high_curvature(self):
        ...
        assert (
            tight_max_curv > gentle_max_curv
...
class TestSmoothingPointCount:
    ...
    def test_point_count_increases(self):
        ...
        assert (
            smooth_count >= expected_count * 0.8
