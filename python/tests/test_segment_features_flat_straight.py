...
class TestFlatStraightPath:
    ...
    def test_curvature_is_zero(self):
        ...
        for seg in analysis.segments:
            assert seg.curvature_1pm < 1e-6, f"Expected curvature ≈ 0, got {seg.curvature_1pm}"
            assert seg.curve_radius_m == float('inf'), "Expected infinite radius for straight"
    ...
    def test_grade_is_zero(self):
        ...
        for seg in analysis.segments[1:]:  # Skip first (no grade calc)
            assert abs(seg.grade_percent) < 0.1, f"Expected grade ≈ 0%, got {seg.grade_percent}%"
    ...
    def test_lateral_accel_is_zero(self):
        ...
        for seg in analysis.segments:
            assert abs(seg.lateral_accel_mps2) < 0.01, f"Expected lat accel ≈ 0, got {seg.lateral_accel_mps2}"
            assert abs(seg.lateral_g) < 0.001, f"Expected lat G ≈ 0, got {seg.lateral_g}"
    ...
    def test_dcurvature_ds_is_zero(self):
        ...
        for seg in analysis.segments[1:]:
            assert abs(seg.dcurvature_ds) < 1e-8, f"Expected dκ/ds ≈ 0, got {seg.dcurvature_ds}"
    ...
    def test_jerk_is_low(self):
        ...
        for seg in analysis.segments[3:]:
            ...
            assert abs(seg.longitudinal_jerk_mps3) < 0.5, f"Expected low jerk, got {seg.longitudinal_jerk_mps3}"
    ...
    def test_comfort_score_is_low(self):
        ...
        for seg in analysis.segments:
            assert seg.comfort_score < 0.3, f"Expected comfortable (< 0.3), got {seg.comfort_score}"
    ...
    def test_friction_usage_is_low(self):
        ...
        for seg in analysis.segments:
            ...
            assert seg.friction_usage < 0.3, f"Expected low friction use, got {seg.friction_usage}"
    ...
    def test_recommended_action_not_brake(self):
        ...
        brake_count = sum(1 for s in analysis.segments if s.recommended_action == "BRAKE")
    ...
    def test_turn_type_is_straight(self):
        ...
        for seg in analysis.segments:
            assert seg.turn_type == "straight", f"Expected 'straight', got '{seg.turn_type}'"
    
    def test_route_level_aggregates(self):
        """Route-level metrics should be reasonable."""
        path = create_flat_straight_path(num_points=50, segment_length_m=10.0, speed_mps=20.0)
        analysis = analyze_profile(path)
        
        # Distance should match
        expected_dist = 49 * 10.0 / 1000  # km
        assert abs(analysis.total_distance_km - expected_dist) < 0.01, \
            f"Expected ~{expected_dist:.3f} km, got {analysis.total_distance_km}"
        
        # Average speed should match input
        assert abs(analysis.avg_speed_kmh - 20 * 3.6) < 1.0, \
            f"Expected ~72 km/h, got {analysis.avg_speed_kmh}"
        
        # No turns on straight path
        assert analysis.turns_per_km < 0.1, f"Expected ~0 turns/km, got {analysis.turns_per_km}"
